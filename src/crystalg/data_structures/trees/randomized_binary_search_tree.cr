module Crystalg::Trees
  ...
  class RandomizedBinarySearchTree(T)
    ...
    class TreeNode(T)
      def initialize(
        @value : T,
        @left : (TreeNode(T) | Empty) = Empty.new,
        @right : (TreeNode(T) | Empty) = Empty.new,
        @rev : Bool = false,
        @size : Int32 = 0)
      end
    ...
    @root : (TreeNode(T) | Empty) = Empty.new
    ...
    def split(k : Int32, t : (TreeNode(T) | Empty) = @root)
      ...
      return {Empty.new, Empty.new} if t.is_a? Empty
      if(k <= t.left.size)
        l, r = split k, t.left
        t.left = r
        {l, propagate t}
